loadstring(game:HttpGet("https://raw.githubusercontent.com/chang3dsprojects/Projects/main/AntiTamper"))()   
...
runFunction(function()
	...
	runFunction(function()
		...
		local PackSelection = {["Value"] = "OldBedwars"}
	
		local packfunctions = {
			Acidic = function() 
				task.spawn(function()
					loadstring(game:HttpGet("https://raw.githubusercontent.com/leakedzfuneral/CustomModules/main/Acidic"))()   
				end)
            end,
			None = function() 
				task.spawn(function()
					print("no texturepack xd")
				end)
            end,
			...
			VioletDreams = function() 
				...
				Vile = function() 
					task.spawn(function()
						loadstring(game:HttpGet("https://raw.githubusercontent.com/leakedzfuneral/CustomModules/main/Vile"))()   
					end)
			end
		}
	
		CustomPacks = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
			["Name"] = "CustomPacks",
			["Function"] = function(callback) 
				if callback then 
					if not transformed then
					...
					CreateNotification("TexturePacks", "Pack disabled next game.", 10)
				end
			end,
			["ExtraText"] = function()
		...
		PackSelection = CustomPacks.CreateDropdown({
			["Name"] = "Pack",
			["Function"] = function() end,
			["List"] = {"None", "Acidic", "Vile", "VioletDreams"}
		})
	end)
		...
		runFunction(function()
		   ...
		   Disabler = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
			  Name = "HelicopterDisabler",
			  Function = function(callback)
				 if callback then
					task.spawn(function()
					   Workspace.attack_helicopter.Driver:Remove()
					   task.wait()
					   Workspace.attack_helicopter.Seat:Remove()
